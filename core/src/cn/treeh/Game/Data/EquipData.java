...
import cn.treeh.Game.Player.EquipStat;
...
public class EquipData {
    ...
    public static EquipData get(int itemid) {
        if (cache.containsKey(itemid))
            return cache.get(itemid);
        EquipData data = new EquipData(itemid);
    ...
    static EquipSlot.Id[] equipslots =
            {
                    EquipSlot.Id.HAT,
                    EquipSlot.Id.FACE,
                    EquipSlot.Id.EYEACC,
    ...
    EquipData(int id) {
        ...
        String strid = "0" + id;
        String category = itemdata.getCategory(id);
        Node src = NXFiles.Character().subNode(category)
                .subNode(strid + ".img").subNode("info");
        ...
        if (index < NON_WEAPON_TYPES) {
            ...
            type = types[index];
            slot = equipslots[index];
        } else if (index >= WEAPON_OFFSET && index < WEAPON_OFFSET + WEAPON_TYPES) {
            int weaponindex = index - WEAPON_OFFSET;
            type = types2[weaponindex];
            slot = EquipSlot.Id.WEAPON;
        } else {
            type = "CASH";
            slot = EquipSlot.Id.NONE;
        }
    }
    public EquipSlot.Id getSlot(){
        return slot;
    }
